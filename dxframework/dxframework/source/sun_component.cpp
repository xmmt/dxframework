...
vertex_shader sun_component::vs_;
pixel_shader sun_component::ps_;
constant_buffer<CB_VS_vertexshader> sun_component::cb_;

sun_component::sun_component(renderer& renderer, std::string file_path)
    : game_component{ renderer }
    , file_path_{ std::move(file_path) } {
    //transform_.set_position({ 10.f, 0.f, 10.f });
    transform_.set_position({ 600.f, 0.f, 600.f });
    //transform_.set_rotation(0.f, DirectX::XM_PIDIV2, 0.f);
    transform_.set_scale(60.f, 60.f, 60.f);
}

void sun_component::destroy_resources() {
}

void sun_component::draw(DirectX::XMMATRIX const& view_projection_matrix) {
    renderer_.set_shaders(vs_, ps_);
    model_.draw(transform_.get_world_matrix(), view_projection_matrix);
}

void sun_component::initialize() {
...
void sun_component::reload() {
}

void sun_component::update() {
    transform_.add_rotation({ 1.f, 1.f, 0.f }, -0.003f);
}

transform& sun_component::get_transform() {
    return transform_;
}