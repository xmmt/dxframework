...
vertex_shader fruit_component::vs_;
pixel_shader fruit_component::ps_;
constant_buffer<CB_VS_vertexshader> fruit_component::cb_;

fruit_component::fruit_component(renderer& renderer, std::string file_path)
    : game_component{ renderer }
    , file_path_{ std::move(file_path) } {
    transform_.set_position({ 0.f, 0.f, 0.f });
    transform_.set_rotation(120.f, 45.f, 45.f);
}

void fruit_component::destroy_resources() {
}

void fruit_component::draw(DirectX::XMMATRIX const& view_projection_matrix) {
    renderer_.set_shaders(vs_, ps_);
    model_.draw(transform_.get_world_matrix(), view_projection_matrix);
}

void fruit_component::initialize() {
...
void fruit_component::reload() {
}

void fruit_component::update() {
    transform_.add_local_rotation({ 0.f, 0.f, 1.f }, 0.03f);
}
