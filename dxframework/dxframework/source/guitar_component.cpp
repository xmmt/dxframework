...
vertex_shader guitar_component::vs_;
pixel_shader guitar_component::ps_;
constant_buffer<CB_VS_vertexshader> guitar_component::cb_;

guitar_component::guitar_component(renderer& renderer, std::string file_path)
    : game_component{ renderer }
    , file_path_{ std::move(file_path) } {
    transform_.set_position({ 0.f, 3.f, 5.f });
    //transform_.set_rotation(0.f, DirectX::XM_PIDIV2, 0.f);
    transform_.set_scale(0.3f, 0.3f, 0.3f);
}

void guitar_component::destroy_resources() {
}

void guitar_component::draw(DirectX::XMMATRIX const& view_projection_matrix) {
    renderer_.set_shaders(vs_, ps_);
    model_.draw(transform_.get_world_matrix(), view_projection_matrix);
}

void guitar_component::initialize() {
...
void guitar_component::reload() {
}

void guitar_component::update() {
